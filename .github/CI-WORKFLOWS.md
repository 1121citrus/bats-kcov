...
| **Test** | After lint (parallel with build) | Run bats suite with static analysis |
...
The Test and Smoke jobs are defined inline because they are specific to this
repo.

### Global configuration

- **Image name:** `1121citrus/bats-kcov`
...
- **Group:** `<workflow-name>-<ref>` — one concurrent run per workflow +
  branch/tag
...
# Publishes: 1121citrus/bats-kcov:1.2.3 + :1.2 + :1 + :latest
```
...
mounts the project source read-only into `bats/bats:1.13.0`. All tests are
static (Dockerfile and README analysis) — no host packages required:

```bash
docker run -i --rm -v "$PWD:/code:ro" -w /code bats/bats:1.13.0 \
  test/01-dockerfile.bats test/02-image-metadata.bats test/03-build.bats
```
...
| ------- | --------------- |
| Tag `v1.2.3` | `1121citrus/bats-kcov:1.2.3` + `:1.2` + `:1` + `:latest` |
| Push to `staging` | `1121citrus/bats-kcov:staging-<sha>` + `:staging` |
...
[Lint] — shared: hadolint + shellcheck + markdownlint
    ↓ (parallel)
[Build] — shared               [Test] — bats static analysis
    ↓ (parallel)
...
`dependabot.yml` configures weekly automated PRs to keep GitHub Actions
current.
