...
- [Bats](https://github.com/bats-core/bats-core) (tests run in the official
  `bats/bats:1.13.0` container — no local installation required)
...
# Or run bats directly with a local bats installation:
...
```bash
shellcheck build test/test_helper.bash
hadolint Dockerfile
```
...
tagged as both `1.2.3` and `latest`, with SLSA provenance and SBOM
attestations.
